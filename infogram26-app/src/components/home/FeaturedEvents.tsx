  ...
  return (
    <section className="section-padding container-xl mx-auto px-4 relative tech-grid">
      <div className="text-center mb-14 relative z-10">
        ...
        <h2 className="text-3xl md:text-5xl font-bold text-slate-900 mb-4" style={{ fontFamily: 'var(--font-display)' }}>
          <span className="gradient-text">Event Categories</span>
        </h2>
        <p className="text-slate-600 max-w-xl mx-auto font-medium" style={{ fontFamily: 'var(--font-heading)' }}>
            ...
            {technicalEvents.map((event, idx) => (
              <motion.div key={event.id} variants={itemVariantsLeft}>
                ...
                >
                  ...
                  <span className="text-slate-900 font-bold text-base sm:text-lg uppercase tracking-[0.08em] group-hover:text-[#7c3aed] transition-colors" style={{ fontFamily: 'var(--font-heading)' }}>
                    {event.name}
                  </span>
        ...
        >
          <div className="p-6 pb-0">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-xl font-bold text-teal-600 uppercase tracking-[0.15em]" style={{ fontFamily: 'var(--font-heading)' }}>
                Non-Technical
              </h3>
            </div>
          </div>
          <div className="px-6 pb-6 space-y-1">
            {nonTechnicalEvents.map((event, idx) => (
              <motion.div key={event.id} variants={itemVariantsRight}>
                <Link
                  href={`/events/${event.slug}`}
                  className="group flex items-center gap-4 py-3 px-4 rounded-xl hover:bg-teal-500/8 transition-all duration-200"
                  style={{ WebkitTapHighlightColor: 'transparent' }}
                >
                  <span className="text-teal-600 font-bold text-sm tabular-nums w-6 shrink-0" style={{ fontFamily: 'var(--font-display)' }}>
                    {idx + 1}.
                  </span>
                  <span className="text-slate-900 font-bold text-base sm:text-lg uppercase tracking-[0.08em] group-hover:text-teal-600 transition-colors" style={{ fontFamily: 'var(--font-heading)' }}>
                    {event.name}
                  </span>
