...
import { LayoutDashboard, CheckSquare, Users, Trophy, LogOut, Menu, X, Home, Calendar, Sun, Moon } from 'lucide-react';
  ...
  if (loading || (!isOrganizer && !isAdmin)) {
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center ${isDark ? 'bg-[#04060f] text-white' : 'bg-slate-50 text-slate-900'} p-4`}>
        ...
        <p className="font-bold text-sm">Loading Event Admin Portal...</p>
      </div>
    );
  }
    ...
    <div className={`min-h-screen flex ${isDark ? 'bg-[#04060f] text-white' : 'bg-slate-100 text-slate-950'}`}>
      ...
      `}>
        <div className="p-5 border-b border-slate-800/50 flex flex-col items-center">
          <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-blue-500/50 bg-blue-950/40 flex items-center justify-center mb-2">
          ...
          <div className="mt-2 text-center text-[10px] font-black uppercase tracking-wider text-blue-300 bg-blue-900/30 py-1 px-3 rounded-full border border-blue-500/30 flex items-center justify-center gap-1">
            <Calendar className="w-3 h-3 text-blue-400" /> EVENT ADMIN
          </div>
        </div>

        {user && (
          <div className="p-4 border-b border-slate-800/40 flex items-center gap-3">
            <div className="w-9 h-9 rounded-full overflow-hidden bg-blue-900/50 border border-blue-500/40 flex items-center justify-center shrink-0">
              {user.photoURL ? (
                <Image src={user.photoURL} alt="Avatar" width={36} height={36} />
              ) : (
                <span className="font-black text-blue-300 text-sm">{user.displayName?.charAt(0) || 'O'}</span>
              )}
            </div>
            <div className="overflow-hidden text-xs">
              <p className={`font-black truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>{user.displayName || 'Event Organizer'}</p>
              <p className="text-blue-400 text-[11px] font-bold truncate">
        ...
        <div className="p-3 border-t border-slate-800/40 space-y-1.5">
          {isAdmin && (
            <Link 
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-purple-400 hover:bg-purple-900/30 transition-colors text-xs font-black uppercase"
            >
